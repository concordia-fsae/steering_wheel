...
{ 
	...
	SPI_2.transfer(0xff); //extra clock for sd card controller
}
...
{
	Sel(); 
	SPI_2.transfer(0xff);//extra clock cycles
	SPI_2.transfer(0x40 | Cmd);
	SPI_2.transfer(0xff & (Lba >> 24));
	SPI_2.transfer(0xff & (Lba >> 16));
	SPI_2.transfer(0xff & (Lba >> 8));
	SPI_2.transfer(0xff & (Lba));
	SPI_2.transfer(Crc); 
	SPI_2.transfer(0xff);//extra clock cycles
}
...
FT_SDStatus FT_SD::R1(uint8_t &r) 
{
	...
	while ((r = SPI_2.transfer(0xff)) & 0x80)
	{
	  i--;
	  if(i <= 0)
	  {
		/* Timeout error */
		DeSel();
		SPI_2.transfer(0xff);   // trailing byte
		return FT_SD_ERROR;
	  }
	}
...
FT_SDStatus FT_SD::Init(void) 
{
	...
	SPI_2.setClockDivider(255);//hardcoding for due board
#else	
...
void FT_SD::Cmd17(uint32_t Off) {
	...
	StatusR1 = R1(r);    
	Sel();
	while (SPI_2.transfer(0xff) != 0xfe)
	  ;
}
...
void FT_SDFile::ReadSector(uint8_t *dst) 
{
	...
	SPI_2.transfer(0xff);   // consume CRC
	SPI_2.transfer(0xff);
	FTSD->DeSel();
}
